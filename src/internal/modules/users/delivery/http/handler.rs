...
use crate::pkg::RepositoryError;
use crate::internal::modules::users::domain::User;
use crate::internal::modules::users::delivery::http::dto::{CreateUserDto, UpdateUserDto, UserResponse, ApiResponse, UserListResponse};
...
/// Convert RepositoryError to HTTP response
...
pub async fn create_user<R: UserRepository>(
    ...
    dto.validate()
        .map_err(|e| RepositoryError::ValidationError(format!("{}", e)))?;
...
pub async fn find_by_username<R: UserRepository>(
    ...
    match user {
        ...
        None => Err(RepositoryError::ValidationError(
